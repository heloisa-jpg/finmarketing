...
export default function RH({ abaInicial = 'overview' }) {
  ...
  const aniversariantes = colaboradores.filter(c => c.aniversario && c.status === 'ativo' && parseISO(c.aniversario).getMonth() === mesAtual).sort((a,b) => parseISO(a.aniversario).getDate() - parseISO(b.aniversario).getDate())
  ...
  return (
    <div>
      ...
      {aba === 'ferias' && <Ferias colaboradores={colaboradores} />}
      {aba === 'desempenho' && <Desempenho colaboradores={colaboradores} />}
      {aba === 'onboarding' && <Onboarding colaboradores={colaboradores} />}
      {aba === 'salarios' && <Salarios colaboradores={colaboradores} />}
      {aba === 'beneficios' && <Beneficios colaboradores={colaboradores} />}
    </div>
  )
}
...
function Overview({ colaboradores, aniversariantes, loading }) {
  const total = colaboradores.filter(c => c.status === 'ativo').length
  const clt = colaboradores.filter(c => c.tipo === 'clt' && c.status === 'ativo').length
  const pj = colaboradores.filter(c => c.tipo === 'pj' && c.status === 'ativo').length
  return (
    <>
      <div className="metric-grid">
        <div className="metric-card"><div className="label">Ativos</div><div className="value">{total}</div></div>
        <div className="metric-card"><div className="label">CLT</div><div className="value">{clt}</div><div className="sub">Ponto + férias</div></div>
        <div className="metric-card"><div className="label">PJ</div><div className="value">{pj}</div><div className="sub">Só cadastro e NF</div></div>
      ...
      <div className="card" style={{ padding: 0, overflow: 'hidden' }}>
        {loading ? <div className="empty"><div className="spinner" style={{ margin: '2rem auto' }} /></div> : colaboradores.length === 0 ? <div className="empty">Nenhum colaborador</div> : (
          <table>
            <thead><tr><th>Nome</th><th>Tipo</th><th>Cargo</th><th>Setor</th><th>Turno</th><th>Admissão</th><th>Status</th></tr></thead>
            <tbody>
              {colaboradores.filter(c => c.status === 'ativo').map(c => (
                <tr key={c.id}>
                  <td><div style={{ display: 'flex', alignItems: 'center', gap: 8 }}><Avatar nome={c.nome} /><div><div style={{ fontWeight: 500, fontSize: 13 }}>{c.nome}</div>{c.email && <div style={{ fontSize: 11, color: 'var(--text3)' }}>{c.email}</div>}</div></div></td>
...
function Colaboradores({ colaboradores, setores, reload }) {
  const [mostrarForm, setMostrarForm] = useState(false)
  const [salvando, setSalvando] = useState(false)
  const [busca, setBusca] = useState('')
  const [filtTipo, setFiltTipo] = useState('')
  const [form, setForm] = useState({ nome:'', tipo:'clt', status:'ativo', cargo:'', setor_id:'', data_admissao:'', data_desligamento:'', aniversario:'', cpf:'', rg:'', cnpj:'', endereco:'', email:'', telefone:'', chave_pix:'', turno:'', hora_entrada:'', hora_saida:'', carga_horaria_dia:'6' })
  const setF = (k,v) => setForm(f => ({...f,[k]:v}))

  const handleSubmit = async (e) => {
    e.preventDefault(); setSalvando(true)
    await supabase.from('colaboradores').insert({ ...form, carga_horaria_dia: parseFloat(form.carga_horaria_dia)||6, setor_id: form.setor_id||null, turno: form.turno||null })
    setSalvando(false); setMostrarForm(false); reload()
    setForm({ nome:'', tipo:'clt', status:'ativo', cargo:'', setor_id:'', data_admissao:'', data_desligamento:'', aniversario:'', cpf:'', rg:'', cnpj:'', endereco:'', email:'', telefone:'', chave_pix:'', turno:'', hora_entrada:'', hora_saida:'', carga_horaria_dia:'6' })
  }

  const filtrados = colaboradores.filter(c => (c.nome.toLowerCase().includes(busca.toLowerCase()) || (c.cargo||'').toLowerCase().includes(busca.toLowerCase())) && (!filtTipo || c.tipo === filtTipo))

  return (
    <>
      <div className="topbar">
        <span style={{ fontSize: 13, color: 'var(--text3)' }}>{filtrados.length} colaboradores</span>
        <button className="btn-primary" onClick={() => setMostrarForm(!mostrarForm)}>{mostrarForm ? '← Fechar' : '+ Novo colaborador'}</button>
      </div>
      {mostrarForm && (
        <div className="card" style={{ marginBottom: 12 }}>
          ...
          <form onSubmit={handleSubmit}>
            <div className="form-grid">
              ...
              <div className="form-group"><label>Tipo *</label><select value={form.tipo} onChange={e => setF('tipo',e.target.value)}><option value="clt">CLT</option><option value="pj">PJ</option><option value="estagio">Estágio</option><option value="freela">Freela</option></select></div>
              <div className="form-group"><label>Status</label><select value={form.status} onChange={e => setF('status',e.target.value)}><option value="ativo">Ativo</option><option value="inativo">Inativo</option><option value="ferias">Férias</option><option value="afastado">Afastado</option></select></div>
              <div className="form-group"><label>Cargo</label><input type="text" value={form.cargo} onChange={e => setF('cargo',e.target.value)} /></div>
              <div className="form-group"><label>Setor</label><select value={form.setor_id} onChange={e => setF('setor_id',e.target.value)}><option value="">Selecionar...</option>{setores.map(s => <option key={s.id} value={s.id}>{s.nome}</option>)}</select></div>
              ...
              <div className="form-group"><label>Desligamento</label><input type="date" value={form.data_desligamento} onChange={e => setF('data_desligamento',e.target.value)} /></div>
              ...
              {form.tipo === 'clt' && <>
                <div className="form-group"><label>Turno</label><select value={form.turno} onChange={e => setF('turno',e.target.value)}><option value="">Selecionar...</option><option value="diurno">Diurno</option><option value="tarde">Tarde</option><option value="noite">Noite</option><option value="madrugada">Madrugada</option></select></div>
      ...
      <div className="filters">
        ...
        <select value={filtTipo} onChange={e => setFiltTipo(e.target.value)}><option value="">Todos os tipos</option><option value="clt">CLT</option><option value="pj">PJ</option><option value="estagio">Estágio</option><option value="freela">Freela</option></select>
      </div>
      <div className="card" style={{ padding: 0, overflow: 'hidden' }}>
        {filtrados.length === 0 ? <div className="empty">Nenhum colaborador encontrado</div> : (
          <table>
            <thead><tr><th>Nome</th><th>Tipo</th><th>Cargo</th><th>Setor</th><th>Admissão</th><th>Aniversário</th><th>Doc.</th><th>Turno</th><th>Status</th></tr></thead>
            <tbody>
              {filtrados.map(c => (
                <tr key={c.id}>
                  <td><div style={{ display:'flex', alignItems:'center', gap:8 }}><Avatar nome={c.nome} /><div><div style={{ fontWeight:500, fontSize:13 }}>{c.nome}</div>{c.email && <div style={{ fontSize:11, color:'var(--text3)' }}>{c.email}</div>}</div></div></td>
                  ...
                  <td style={{ fontFamily:'DM Mono', fontSize:11, color:'var(--text3)' }}>{c.cpf||c.cnpj||'—'}</td>
                  <td style={{ fontSize:12, color:'var(--text3)' }}>{c.tipo==='clt' ? (TURNO_LABEL[c.turno]||'—') : '—'}</td>
                  <td><span className={`badge ${STATUS_CLS[c.status]}`}>{STATUS_LABEL[c.status]}</span></td>
                </tr>
              ))}
...
function Ponto({ colaboradores }) {
  ...
  const load = async () => {
    setLoading(true)
    const { data } = await supabase.from('pontos').select('*, colaboradores(nome,turno,carga_horaria_dia)').eq('data', hoje)
    setRegistros(data||[])
    setLoading(false)
  }
  ...
  const bater = async (colabId, tipo) => {
    ...
    if (!reg) {
      ...
      if (tipo === 'pausa') upd.inicio_pausa = agora
        ...
        const carga = reg.colaboradores?.carga_horaria_dia || 6
        upd.total_horas = totalH
        upd.horas_extras = Math.max(0, Math.round((totalH - carga) * 100) / 100)
  ...
  return (
    <div>
      <div className="topbar"><h2 style={{ fontSize:15, fontWeight:500 }}>Ponto — {format(new Date(), "EEEE dd/MM/yyyy", { locale: ptBR })}</h2></div>
      <div className="card" style={{ padding:0, overflow:'hidden' }}>
        {loading ? <div className="empty"><div className="spinner" style={{ margin:'2rem auto' }} /></div>
          ...
          : (
            <table>
              <thead><tr><th>Colaborador</th><th>Turno</th><th>Entrada</th><th>Pausa</th><th>Retorno</th><th>Saída</th><th>Total</th><th>Banco de horas</th><th>Ação</th></tr></thead>
              <tbody>
                {colaboradores.map(c => {
                    ...
                    <tr key={c.id}>
                      ...
                      <td>
                        <div style={{ display:'flex', gap:4, flexWrap:'wrap' }}>
                          ...
                          {ok && <span className="badge badge-green">✓</span>}
                        </div>
                      </td>
                    </tr>
                  )
...
function Ferias({ colaboradores }) {
  ...
  const handleSubmit = async (e) => {
    ...
    await supabase.from('ferias_rh').insert({ ...form, dias: Math.round((d2-d1)/86400000)+1, status:'aguardando' })
  ...
  return (
    <div>
      <div className="topbar"><h2 style={{ fontSize:15, fontWeight:500 }}>Férias e ausências</h2><button className="btn-primary" onClick={() => setMostrarForm(!mostrarForm)}>{mostrarForm?'← Fechar':'+ Solicitar'}</button></div>
      {mostrarForm && (
        <div className="card" style={{ marginBottom:12 }}>
          <form onSubmit={handleSubmit}>
            <div className="form-grid">
              <div className="form-group"><label>Colaborador *</label><select value={form.colaborador_id} onChange={e => setForm(f=>({...f,colaborador_id:e.target.value}))} required><option value="">Selecionar...</option>{colaboradores.filter(c=>c.tipo==='clt').map(c=><option key={c.id} value={c.id}>{c.nome}</option>)}</select></div>
              <div className="form-group"><label>Tipo</label><select value={form.tipo} onChange={e => setForm(f=>({...f,tipo:e.target.value}))}><option value="ferias">Férias</option><option value="falta">Falta</option><option value="atestado">Atestado</option><option value="licenca">Licença</option><option value="folga">Folga</option><option value="falta_justificada">Falta justificada</option></select></div>
            ...
            <div style={{ display:'flex', gap:8, marginTop:12 }}>
              <button type="submit" className="btn-primary" disabled={salvando}>{salvando?'Salvando...':'Solicitar'}</button>
              <button type="button" className="btn-ghost" onClick={() => setMostrarForm(false)}>Cancelar</button>
            </div>
          </form>
        </div>
      ...
      <div className="card" style={{ padding:0, overflow:'hidden' }}>
        {loading ? <div className="empty"><div className="spinner" style={{ margin:'2rem auto' }} /></div>
          ...
          : <table><thead><tr><th>Colaborador</th><th>Tipo</th><th>Início</th><th>Fim</th><th>Dias</th><th>Status</th><th>Ações</th></tr></thead><tbody>
            {dados.map(f=>(
              <tr key={f.id}>
                ...
                <td>{f.status==='aguardando'&&<div style={{ display:'flex', gap:5 }}><button className="btn-success" style={{ padding:'3px 8px', fontSize:11 }} onClick={()=>aprovar(f.id,'aprovado')}>Aprovar</button><button className="btn-danger" style={{ padding:'3px 8px', fontSize:11 }} onClick={()=>aprovar(f.id,'recusado')}>Recusar</button></div>}</td>
              </tr>
            ))}
          </tbody></table>}
      </div>
...
function Desempenho({ colaboradores }) {
  const [dados, setDados] = useState([])
  const [loading, setLoading] = useState(true)
  const [mostrarForm, setMostrarForm] = useState(false)
  const [salvando, setSalvando] = useState(false)
  ...
  return (
    <div>
      <div className="topbar"><h2 style={{ fontSize:15, fontWeight:500 }}>Avaliação de desempenho</h2><button className="btn-primary" onClick={() => setMostrarForm(!mostrarForm)}>{mostrarForm?'← Fechar':'+ Nova avaliação'}</button></div>
      {mostrarForm && (
        <div className="card" style={{ marginBottom:12 }}>
          <form onSubmit={handleSubmit}>
            <div className="form-grid">
              <div className="form-group"><label>Colaborador *</label><select value={form.colaborador_id} onChange={e=>setForm(f=>({...f,colaborador_id:e.target.value}))} required><option value="">Selecionar...</option>{colaboradores.map(c=><option key={c.id} value={c.id}>{c.nome}</option>)}</select></div>
            ...
            <div style={{ display:'flex', gap:8, marginTop:12 }}><button type="submit" className="btn-primary" disabled={salvando}>{salvando?'Salvando...':'Salvar'}</button><button type="button" className="btn-ghost" onClick={() => setMostrarForm(false)}>Cancelar</button></div>
          </form>
        </div>
      )}
      <div className="card" style={{ padding:0, overflow:'hidden' }}>
        {loading ? <div className="empty"><div className="spinner" style={{ margin:'2rem auto' }} /></div>
          ...
          : <table><thead><tr><th>Colaborador</th><th>Ciclo</th><th>Nota gestor</th><th>Autoaval.</th><th>Pontos fortes</th><th>Desenvolvimento</th><th>Status</th></tr></thead><tbody>
            {dados.map(a=>(
              <tr key={a.id}>
...
function Onboarding({ colaboradores }) {
  const [dados, setDados] = useState([])
  const [loading, setLoading] = useState(true)
  ...
  return (
    <div>
      <div className="topbar">
        <h2 style={{ fontSize:15, fontWeight:500 }}>Onboarding</h2>
        <div style={{ display:'flex', gap:8, alignItems:'center' }}>
          <select value={criando} onChange={e=>setCriando(e.target.value)} style={{ fontSize:12 }}><option value="">Selecionar colaborador...</option>{colaboradores.filter(c=>!colabs.includes(c.id)).map(c=><option key={c.id} value={c.id}>{c.nome}</option>)}</select>
...
function Salarios({ colaboradores }) {
  const [dados, setDados] = useState([])
  const [loading, setLoading] = useState(true)
  const [mostrarForm, setMostrarForm] = useState(false)
  const [salvando, setSalvando] = useState(false)
  const [form, setForm] = useState({ colaborador_id:'', valor:'', data_vigencia:format(new Date(),'yyyy-MM-dd'), percentual_reajuste:'', observacoes:'' })

  useEffect(() => { load() }, [])
  const load = async () => {
    setLoading(true)
    const { data } = await supabase.from('salarios').select('*, colaboradores(nome,tipo)').order('data_vigencia',{ascending:false})
    setDados(data||[])
    setLoading(false)
  }
  const handleSubmit = async (e) => {
    e.preventDefault(); setSalvando(true)
    await supabase.from('salarios').insert({ ...form, valor:parseFloat(form.valor), percentual_reajuste:parseFloat(form.percentual_reajuste)||null })
    setSalvando(false); setMostrarForm(false); load()
  }

  return (
    <div>
      <div className="topbar"><h2 style={{ fontSize:15, fontWeight:500 }}>Salários e reajustes</h2><button className="btn-primary" onClick={()=>setMostrarForm(!mostrarForm)}>{mostrarForm?'← Fechar':'+ Registrar'}</button></div>
      {mostrarForm && (
        <div className="card" style={{ marginBottom:12 }}>
          <form onSubmit={handleSubmit}>
            <div className="form-grid">
              <div className="form-group"><label>Colaborador *</label><select value={form.colaborador_id} onChange={e=>setForm(f=>({...f,colaborador_id:e.target.value}))} required><option value="">Selecionar...</option>{colaboradores.map(c=><option key={c.id} value={c.id}>{c.nome}</option>)}</select></div>
              <div className="form-group"><label>Valor (R$) *</label><input type="number" step="0.01" value={form.valor} onChange={e=>setForm(f=>({...f,valor:e.target.value}))} required /></div>
              <div className="form-group"><label>Vigência</label><input type="date" value={form.data_vigencia} onChange={e=>setForm(f=>({...f,data_vigencia:e.target.value}))} /></div>
              <div className="form-group"><label>% reajuste</label><input type="number" step="0.1" value={form.percentual_reajuste} onChange={e=>setForm(f=>({...f,percentual_reajuste:e.target.value}))} placeholder="Ex: 12" /></div>
              <div className="form-group full"><label>Observações</label><input type="text" value={form.observacoes} onChange={e=>setForm(f=>({...f,observacoes:e.target.value}))} /></div>
            </div>
            <div style={{ display:'flex', gap:8, marginTop:12 }}><button type="submit" className="btn-primary" disabled={salvando}>{salvando?'Salvando...':'Salvar'}</button><button type="button" className="btn-ghost" onClick={()=>setMostrarForm(false)}>Cancelar</button></div>
          </form>
      ...
      <div className="card" style={{ padding:0, overflow:'hidden' }}>
        {loading?<div className="empty"><div className="spinner" style={{ margin:'2rem auto' }}/></div>:dados.length===0?<div className="empty">Nenhum registro</div>:(
          <table><thead><tr><th>Colaborador</th><th>Tipo</th><th>Salário</th><th>Vigência</th><th>Reajuste</th><th>Observações</th></tr></thead><tbody>
            {dados.map(s=>(
              <tr key={s.id}>
                <td style={{ fontWeight:500 }}>{s.colaboradores?.nome}</td>
                <td><span className={`badge ${TIPO_CLS[s.colaboradores?.tipo]}`}>{s.colaboradores?.tipo?.toUpperCase()}</span></td>
                <td style={{ fontFamily:'DM Mono', fontWeight:600 }}>{fmtVal(s.valor)}</td>
                <td style={{ fontFamily:'DM Mono', fontSize:12, color:'var(--text3)' }}>{s.data_vigencia?format(parseISO(s.data_vigencia),'MM/yyyy'):'—'}</td>
                <td style={{ color:'var(--green)', fontSize:12 }}>{s.percentual_reajuste?`+${s.percentual_reajuste}%`:'—'}</td>
                <td style={{ fontSize:12, color:'var(--text3)' }}>{s.observacoes||'—'}</td>
              </tr>
            ))}
          </tbody></table>
        )}
...
function Beneficios({ colaboradores }) {
  const [dados, setDados] = useState([])
  const [loading, setLoading] = useState(true)
  const [mostrarForm, setMostrarForm] = useState(false)
  const [salvando, setSalvando] = useState(false)
  const [form, setForm] = useState({ colaborador_id:'', tipo:'vt', descricao:'', valor:'' })
  const TIPO_BEN = { vt:'Vale Transporte', vr:'Vale Refeição', saude:'Plano Saúde', odonto:'Odonto', outros:'Outros' }

  useEffect(() => { load() }, [])
  const load = async () => {
    setLoading(true)
    const { data } = await supabase.from('beneficios_rh').select('*, colaboradores(nome,tipo)').eq('ativo',true).order('created_at')
    setDados(data||[])
    setLoading(false)
  }
  const handleSubmit = async (e) => {
    e.preventDefault(); setSalvando(true)
    await supabase.from('beneficios_rh').insert({ ...form, valor:parseFloat(form.valor)||null })
    setSalvando(false); setMostrarForm(false); load()
  }

  return (
    <div>
      <div className="topbar"><h2 style={{ fontSize:15, fontWeight:500 }}>Benefícios</h2><button className="btn-primary" onClick={()=>setMostrarForm(!mostrarForm)}>{mostrarForm?'← Fechar':'+ Adicionar'}</button></div>
      {mostrarForm && (
        <div className="card" style={{ marginBottom:12 }}>
          <form onSubmit={handleSubmit}>
            <div className="form-grid">
              <div className="form-group"><label>Colaborador *</label><select value={form.colaborador_id} onChange={e=>setForm(f=>({...f,colaborador_id:e.target.value}))} required><option value="">Selecionar...</option>{colaboradores.filter(c=>c.tipo==='clt').map(c=><option key={c.id} value={c.id}>{c.nome}</option>)}</select></div>
              <div className="form-group"><label>Tipo</label><select value={form.tipo} onChange={e=>setForm(f=>({...f,tipo:e.target.value}))}>{Object.entries(TIPO_BEN).map(([k,v])=><option key={k} value={k}>{v}</option>)}</select></div>
              <div className="form-group"><label>Descrição</label><input type="text" placeholder="Ex: Bradesco, R$ 35/dia..." value={form.descricao} onChange={e=>setForm(f=>({...f,descricao:e.target.value}))} /></div>
              <div className="form-group"><label>Valor (R$)</label><input type="number" step="0.01" value={form.valor} onChange={e=>setForm(f=>({...f,valor:e.target.value}))} /></div>
            </div>
            <div style={{ display:'flex', gap:8, marginTop:12 }}><button type="submit" className="btn-primary" disabled={salvando}>{salvando?'Salvando...':'Salvar'}</button><button type="button" className="btn-ghost" onClick={()=>setMostrarForm(false)}>Cancelar</button></div>
          </form>
        </div>
      ...
      <div className="card" style={{ padding:0, overflow:'hidden' }}>
        {loading?<div className="empty"><div className="spinner" style={{ margin:'2rem auto' }}/></div>:dados.length===0?<div className="empty">Nenhum benefício</div>:(
          <table><thead><tr><th>Colaborador</th><th>Tipo</th><th>Descrição</th><th>Valor</th></tr></thead><tbody>
            {dados.map(b=>(
              <tr key={b.id}>
                <td style={{ fontWeight:500 }}>{b.colaboradores?.nome}</td>
                <td><span className="badge badge-blue">{TIPO_BEN[b.tipo]}</span></td>
                <td style={{ fontSize:12, color:'var(--text3)' }}>{b.descricao||'—'}</td>
                <td style={{ fontFamily:'DM Mono', fontSize:13 }}>{fmtVal(b.valor)}</td>
              </tr>
            ))}
          </tbody></table>
        )}
