...
const NAV = [
  { key: 'dashboard', label: 'Painel', icon: '◼', group: 'principal' },
  { key: 'lancamentos', label: 'Despesas', icon: '≡', group: 'principal' },
  { key: 'novo_lancamento', label: 'Lançar gasto', icon: '+', group: 'principal' },
  { key: 'reembolsos', label: 'Reembolsos', icon: '↩', group: 'principal' },
  ...
  { key: 'relatorio', label: 'Relatórios', icon: '▦', group: 'relatorios', adminOnly: true },
    ...
    <div style={{ display: 'flex', height: '100vh', overflow: 'hidden' }}>
      ...
      <aside style={{
              ...
              {visibleNav.filter(n => n.group === group).map(n => (
                <button
                  ...
                  style={{
                    ...
                    background: page === n.key ? 'rgba(200,241,53,.08)' : 'transparent',
        ...
        <div style={{
          ...
          {!collapsed && (
            <button onClick={logout} title="Sair" style={{ background: 'transparent', color: 'var(--text3)', fontSize: 14, padding: 2 }}>
              ⏻
            </button>
          )}
        </div>
